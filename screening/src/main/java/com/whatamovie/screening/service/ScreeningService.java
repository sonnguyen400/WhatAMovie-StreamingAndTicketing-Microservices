...
public class ScreeningService {
    ...
    public Screening insert(ScreeningRequestVm screeningVm) {
        ...
        if (movie != null && auditorium != null) {
            List<Screening> screenings = screeningRepository.findByApproximatelyTime(screening.getStartTime(),screening.getAuditorium_id());
            screenings.sort(Comparator.comparing(Screening::getStartTime));
            if (!screenings.isEmpty()) {
                if (screenings.size() == 1) {
                    if (screening.getStartTime().isAfter(screenings.getFirst().getStartTime()))
                        screenings.addLast(null);
                    else if (screening.getStartTime().isBefore(screenings.getFirst().getStartTime()))
                        screenings.addFirst(null);
                ...
                if (screenings.getFirst() != null) {
                    ...
                    if (!completedTimeOfLastScreening.isBefore(screening.getStartTime()))
                        throw new ConflictScreeningTimeException(String.format("A movie with id{%s} is shown at{%s} is conflict to this movie with start time at %s", lastMovie.getId(), screenings.getFirst().getStartTime(), screening.getStartTime()));
                }
                if (screenings.getLast() != null) {
                    ...
                    if (!completedTimeOfThisScreening.isBefore(screenings.getLast().getStartTime()))
                        throw new ConflictScreeningTimeException(String.format("A movie with id{%s} is shown at{%s} is conflict to this movie with end time at %s", screenings.getLast().getMovie_id(), screenings.getLast().getStartTime(), completedTimeOfThisScreening));
                }
            }
    ...
    public List<Screening> findAllIdIn(List<Long> id) {
        return screeningRepository.findAllByIdIn(id);
    }

    public Screening findById(Long id) {
        return screeningRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException(String.format("Screening with id{%s} not found", id)));
    }
}
